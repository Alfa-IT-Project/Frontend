...
const PayrollCheckModal = ({ 
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('PayrollCheckModal received record:', record);
      console.log('Record values check:', {
        id: record?.id,
        basicSalary: record?.basicSalary,
        allowances: record?.allowances,
        deductions: record?.deductions,
        netSalary: record?.netSalary,
        basicSalaryType: typeof record?.basicSalary,
        netSalaryType: typeof record?.netSalary
      });
    }
  }, [isOpen, record]);
  
  // Safety check - if record is invalid, show error and allow closing
  if (!record || typeof record !== 'object') {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Error">
        <div className="p-4">
          <p className="text-red-500">Invalid payroll data received.</p>
...
export default PayrollCheckModal; 